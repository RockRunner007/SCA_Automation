...
def get_token(xrayurl):
    headers = {'Content-Type': 'application/json'}
    payload = {
        'name': os.environ['xray_user'],
        'password': os.environ['xray_pwd']
    }

    resp = requests.post(f'{xrayurl}/api/v1/auth/token', json=payload, headers=headers, verify=False)
